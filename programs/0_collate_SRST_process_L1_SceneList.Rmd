...
filtered = data %>% 
  ...
  filter_at(vars(med_Red, med_Green, med_Blue, med_Nir, med_Swir1, med_Swir2), all_vars(.<0.2 & . > 0.0))
```
...
```{r}
write.csv(filtered, file.path(data_dir, paste0('SkyLoch_LandsatC2_SRST_filtered_gt0_v', Sys.Date(), '.csv')), row.names = F)
```
